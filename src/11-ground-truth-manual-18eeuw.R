...
files$path_image <- file.path("data/transkribus-lines/HTR test corpus/text_en_foto", files$image)
...
for(i in seq_along(paths)){
  ...
  path <- file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "overview", sprintf("%s.png", subject_id))
  ...
  for(j in seq_len(nrow(traindata))){
    ...
    path <- file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "textlines", sprintf("%s.jpg", path))
...
writeLines(sprintf("%s %s", x$doc_id, x$text), con = file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "tr.txt"))
x <- subset(modeldata, dataset == "validation")
writeLines(sprintf("%s %s", x$doc_id, x$text), con = file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "va.txt"))
x <- subset(modeldata, dataset == "test")
writeLines(sprintf("%s %s", x$doc_id, x$text), con = file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "te.txt"))
x <- sapply(strsplit(readLines( file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "te.txt")), " "), head, 1)
writeLines(x[1:3], file.path("data", "transkribus-lines", "HTR test corpus", "imgs", "test.txt"))
save(transcriptions, file = "data/transkribus-lines/HTR test corpus/transcriptions.RData")

## Also make images with fixed height: 128
x <- list.files("data/transkribus-lines/HTR test corpus/imgs/textlines", full.names = TRUE)
x <- mapply(img = x, path = file.path("data/transkribus-lines/HTR test corpus/imgs/textlines_h128", basename(x)), FUN=function(img, path){
  img <- image_read(img)
...
x <- list.files("data/transkribus-lines/HTR test corpus/imgs/textlines", full.names = TRUE)
x <- mapply(img = x, path = file.path("data/transkribus-lines/HTR test corpus/imgs/textlines_su_h128", basename(x)), FUN=function(img, path){
  img <- image_read(img)
  img <- image_resize(img, "x128")
